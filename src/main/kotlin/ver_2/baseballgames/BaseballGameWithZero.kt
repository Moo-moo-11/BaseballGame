...
class BaseballGameWithZero: AbstractBaseballGame() {

    override fun printPlayResult() {

        if (gamePlayResultMap.isNotEmpty()) {
            println("⚾⚾⚾ 0이 포함된 야구게임 기록입니다 ⚾⚾⚾")
    ...
    override fun playGame() {
        ...
        val answerNumbArray = getRandomThreeNumbArrayWithZero()

        println("[안내] 투수가 공을 던지면 0~9까지의 임의의 수 3개를 순서대로 입력해주세요")
        println("[주의] 첫 번째 숫자는 0이 될 수 없습니다 ⚾⚾⚾")
        ...
        while (true) {
            ...
            val inputArray = getThreeNumbInputArrayWithZero()
    ...
    private fun getThreeNumbInputArrayWithZero():IntArray {

        while (true) {

            val inputString = readln()

            if (Regex("[0-9][0-9][0-9]") matches inputString) {
                if (inputString[0] == inputString[1] || inputString[0] == inputString[2]
                    || inputString[1] == inputString[2]
                ) {
                    println("서로 다른 3개의 숫자를 입력해야합니다. 다시 입력해주세요.")
                } else if (inputString[0] == '0') {
                    println("맨 앞 숫자는 0이 될 수 없습니다. 다시 입력해주세요.")
                } else {
    ...
    private fun getRandomThreeNumbArrayWithZero():IntArray {
